...
interface PostulacionesModalProps {
  ...
  onClose: () => void
  solicitudId: string
  onAceptar: (ofertaId: string, profesionalId: string, nombreProfesional: string) => void
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-lg h-[90vh] overflow-y-auto relative">
        ...
        <h2 className="text-xl font-bold text-indigo-primary mb-4">Postulaciones recibidas</h2>
        {loading ? (
          <div className="text-gray-600 text-center py-12">Cargando ofertas...</div>
        ) : ofertas.length === 0 ? (
          <div className="text-gray-400 text-center py-12">No hay postulaciones para esta solicitud.</div>
        ) : (
          <div className="space-y-4">
            {ofertas.map((oferta) => (
              <div key={oferta.id} className="border rounded-lg p-4 flex flex-col gap-2 bg-gray-50 shadow">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="font-semibold text-indigo-900">{oferta.nombreProfesional}</div>
                ...
                <div>
                  ...
                  <div className="text-gray-900">{oferta.mensaje || <span className="italic text-gray-400">Sin mensaje</span>}</div>
                </div>
                <div className="text-xs text-gray-400">
                  {oferta.createdAt && oferta.createdAt.toDate
                ...
                <button
                  onClick={() => {
                    ...
                    onAceptar(oferta.id, oferta.profesionalId, oferta.nombreProfesional)
                  }}
                  disabled={!!aceptando}
